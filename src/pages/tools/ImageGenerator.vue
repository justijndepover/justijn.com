    ...
    <tool-page name="Image Generator" description="Generate a placeholder image with a specific width & height">
        <div class="rounded-lg border border-gray-300 dark:border-gray-800 p-4 mb-4">
            <form @submit.prevent="setCanvas">
                ...
                <text-input label="Enter the svg width" placeholder="256" v-model="svgWidth" class="mb-4" />
        ...
        <div class="hidden">
            <svg ref="svg" :width="svgWidth" :height="svgWidth" viewBox="0 0 511 511" xmlns="http://www.w3.org/2000/svg">
...
export default {
    ...
    methods: {
        ...
        setCanvas() {
            ...
            ctx.fillStyle = this.svgColor;
            const xml = new XMLSerializer().serializeToString(this.$refs.svg);
            const img = document.createElement('img');
            img.src = `data:image/svg+xml;base64,${btoa(xml)}`;

            img.onload = () => {
                ctx.drawImage(img, (this.width / 2 - this.svgWidth / 2), (this.height / 2 - this.svgWidth / 2), this.svgWidth, this.svgWidth);
